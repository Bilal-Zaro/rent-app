...
// import { toggleAvailability } from "../Redux/availabilitySlice";

const Cheackout = () => {
  const dispatch = useDispatch();
  // const isAvailable = useSelector((state) => state.availability.available); // جلب حالة التوافر
  ...
  const handleSubmit = (e) => {
    ...
    emailjs
      .send(
        "service_jrtimzn", // استبدل بـ Service ID الصحيح
        "template_p1kr1jg", // استبدل بـ Template ID الصحيح
        {
          name: formData.name,
          email: formData.email,
          to_email: formData.to_email, // سيتم إرسال البريد إلى هذا العنوان
          message: formData.message,
        },
        "Uqobi9sYP10iyftG-" // استخدم الـ Public Key الصحيح
      )
      .then(
        (result) => {
          console.log("تم إرسال الرسالة بنجاح:", result.text);
          alert("تم إرسال الرسالة إلى: " + formData.to_email);
        },
        (error) => {
          console.log("فشل في إرسال الرسالة:", error.text);
  ...
  return (
    <div className="flex space-x-6">
      ...
      <div className="max-w-xl mx-auto bg-gray-100 p-6 rounded-lg shadow-sm border-l-4 border-blue-500 mt-6 ml-12">
      ...
      <div className="max-w-lg w-1/2 mx-auto bg-white p-6 rounded-lg shadow-md">
